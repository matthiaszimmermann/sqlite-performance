...
300k nodes seems to correspond to mid-larger sized hyper scaler data centers, although it is difficult to find direct sources for these numbers. 
...
**Total system**: ~663K entities, ~15.3M attributes (0.83x Mendoza benchmark capacity)

## Marketplace Dynamics

The **balanced marketplace model** assumes supply and demand in reasonable equilibrium:
- 300K total nodes, 195K running workloads (65% utilization)
- 105K pending queue = **2.7 hour wait time** (healthy marketplace)
- Dynamic pricing adjusts to maintain this balance

Alternative scenarios show why this matters:
- **Demand-heavy** (GPU shortage): 400K pending, 20h wait → aggressive matching needed
- **Supply-heavy** (excess capacity): 5K pending, <30min wait → nodes sit idle, prices drop

The balanced model represents a **mature, sustainable marketplace** with reasonable queue depths and good resource utilization.
...
```
New submissions:       40 workloads/sec (create)
Scheduling (assign):   11 workloads/sec (update)
Completions:           11 workloads/sec (update)
Failures:              3 workloads/sec (update)
Housekeeping:          40 workloads/sec (cleanup completed for steady-state)
Resource updates:      50-100 workloads/sec (conditional: usage change >20%)
                       ─────────────────────────────────────────
Total steady-state:    ~115-165 workload entities/sec
```
...
| **Resource usage spike** | 54-69 entities/sec | 450 entities/sec | **~504-519 entities/sec** | 30-60 sec, occasional |
| **Mass node failure** | 300 entities/sec | 115-165 entities/sec | **~415-465 entities/sec** | <10 sec, rare |
| **Mass job completion** | 54-69 entities/sec | 300 entities/sec | **~354-369 entities/sec** | ~10 sec, occasional |
...
**Core Challenge:** Match 105K pending workloads to 90K available nodes efficiently in a 300K node compute marketplace.

**Key Constraints:**
- **Matching pool**: 90K available nodes (195K busy, 15K offline)
- **Workload throughput**: 10.8 workloads/sec = 39K/hour
...
1. **Minimize query count**: Avoid per-workload queries (10.8 queries/sec), use batch approach
...
|----------|---------------------|-------------------|-------|
| **Steady-state** | 170-235 | 105-240 | Balanced marketplace, event-driven monitoring |
| **Batch submission** | 255-270 | 200 | Block rate limited (2-sec blocks) |
| **Resource usage spike** | 504-519 | 105-240 | At block capacity, creates temporary backlog |
| **Mass job completion** | 354-369 | 200 | Large batch finishes |
| **Dashboard spike** | 170-235 | 500 | Many users checking status |
...
- **Sustained target**: **500 entities/sec**
  - Rationale: Resource usage spikes (504-519/sec) represent the highest realistic sustained load, occurring when many workloads simultaneously change resource consumption. This matches exactly the block capacity constraint (1,000 entities per 2-sec block).
  - Margin: Steady-state (170-235/sec) runs at 34-47% of capacity, providing substantial headroom for normal operations.
...
- All targets are **well within SQLite capability** (benchmark: ~700 entities/sec)
...
| **Commit latency (p99)** | <1500ms | Block production constraint |
...
| **Query latency (p99)** | <100ms | Complex multi-constraint filters |
| **Result set size** | 10-100 rows typical, 1,000 max | Application-side final selection |
...
1. **Batch write testing**: Verify 500 entities/sec sustained throughput with 500-1,000 entity transactions
2. **Latency under load**: Confirm p99 commit latency <500ms at sustained write rates
