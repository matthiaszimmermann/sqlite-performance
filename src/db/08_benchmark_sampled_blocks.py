#!/usr/bin/env python3
"""Benchmark using sampled blocks from real arkiv data.

This script samples real blocks from a source database and replays
them into an output database, measuring per-block commit performance.

Filters:
- Excludes empty blocks
- Excludes blocks with >500 entities

For each entity, a fresh entity_key is generated to avoid conflicts.

Usage:
    uv run python -m db.08_benchmark_sampled_blocks <sample_db> <base_db> <output_db> <num_blocks>
    
Arguments:
    sample_db  - Source database to sample blocks from (e.g., mendoza dump)
    base_db    - Starting database ("" for fresh start, or existing DB path)
    output_db  - Output database (will be created or appended to)
    num_blocks - Number of blocks to add

Examples:
    # Fresh start (empty base)
    uv run python -m db.08_benchmark_sampled_blocks ./data/mendoza.db "" ./data/sampled_100.db 100
    
    # Continue from existing DB into new file
    uv run python -m db.08_benchmark_sampled_blocks ./data/mendoza.db ./data/sampled_100.db ./data/sampled_200.db 100
    
    # Append in place (same base and output)
    uv run python -m db.08_benchmark_sampled_blocks ./data/mendoza.db ./data/sampled_100.db ./data/sampled_100.db 100

Output:
    - Creates/appends to <output_db>
    - Creates CSV file <output_db>.csv with per-block metrics
"""

import shutil
import sqlite3
import sys
import time
import random
import uuid
from pathlib import Path


# Block filter settings
MIN_ENTITIES_PER_BLOCK = 1
MAX_ENTITIES_PER_BLOCK = 500


def get_valid_blocks(source_conn: sqlite3.Connection) -> list[tuple[int, int]]:
    """Get list of (from_block, entity_count) for valid blocks."""
    cursor = source_conn.execute("""
        SELECT from_block, COUNT(DISTINCT entity_key) as entity_count
        FROM payloads
        GROUP BY from_block
        HAVING entity_count >= ? AND entity_count <= ?
        ORDER BY from_block
    """, (MIN_ENTITIES_PER_BLOCK, MAX_ENTITIES_PER_BLOCK))
    return cursor.fetchall()


def create_arkiv_schema(conn: sqlite3.Connection) -> None:
    """Create the full arkiv schema (bi-temporal, 13 indexes)."""
    conn.execute("""
        CREATE TABLE IF NOT EXISTS string_attributes (
            entity_key BLOB NOT NULL,
            from_block INTEGER NOT NULL,
            to_block INTEGER NOT NULL,
            key TEXT NOT NULL,
            value TEXT NOT NULL,
            PRIMARY KEY (entity_key, key, from_block)
        )
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS sa_ekv_idx ON string_attributes (from_block, to_block, key, value)")
    conn.execute("CREATE INDEX IF NOT EXISTS sa_kv_idx ON string_attributes (key, value, from_block DESC, to_block DESC)")
    conn.execute("CREATE INDEX IF NOT EXISTS sa_ek_idx ON string_attributes (from_block, to_block, key)")
    conn.execute("CREATE INDEX IF NOT EXISTS sa_del_idx ON string_attributes (to_block)")
    conn.execute("CREATE INDEX IF NOT EXISTS sa_ekv2_idx ON string_attributes (entity_key, key, from_block DESC)")
    
    conn.execute("""
        CREATE TABLE IF NOT EXISTS numeric_attributes (
            entity_key BLOB NOT NULL,
            from_block INTEGER NOT NULL,
            to_block INTEGER NOT NULL,
            key TEXT NOT NULL,
            value INTEGER NOT NULL,
            PRIMARY KEY (entity_key, key, from_block)
        )
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS na_ekv_idx ON numeric_attributes (from_block, to_block, key, value)")
    conn.execute("CREATE INDEX IF NOT EXISTS na_ek_idx ON numeric_attributes (from_block, to_block, key)")
    conn.execute("CREATE INDEX IF NOT EXISTS na_kv_idx ON numeric_attributes (key, value, from_block DESC, to_block DESC)")
    conn.execute("CREATE INDEX IF NOT EXISTS na_del_idx ON numeric_attributes (to_block)")
    
    conn.execute("""
        CREATE TABLE IF NOT EXISTS payloads (
            entity_key BLOB NOT NULL,
            from_block INTEGER NOT NULL,
            to_block INTEGER NOT NULL,
            payload BLOB NOT NULL,
            content_type TEXT NOT NULL DEFAULT '',
            string_attributes TEXT NOT NULL DEFAULT '{}',
            numeric_attributes TEXT NOT NULL DEFAULT '{}',
            PRIMARY KEY (entity_key, from_block)
        )
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS p_ek_idx ON payloads (entity_key, from_block, to_block)")
    conn.execute("CREATE INDEX IF NOT EXISTS p_del_idx ON payloads (to_block)")
    
    conn.commit()


def get_block_data(source_conn: sqlite3.Connection, from_block: int) -> dict:
    """Extract all data for a specific block from source database."""
    # Get all entities in this block
    entities = source_conn.execute("""
        SELECT DISTINCT entity_key FROM payloads WHERE from_block = ?
    """, (from_block,)).fetchall()
    
    entity_keys = [e[0] for e in entities]
    
    if not entity_keys:
        return {"entity_keys": [], "string_attrs": [], "numeric_attrs": [], "payloads": []}
    
    # Get string attributes for these entities at this block
    placeholders = ",".join("?" * len(entity_keys))
    string_attrs = source_conn.execute(f"""
        SELECT entity_key, from_block, to_block, key, value
        FROM string_attributes
        WHERE entity_key IN ({placeholders}) AND from_block = ?
    """, (*entity_keys, from_block)).fetchall()
    
    # Get numeric attributes
    numeric_attrs = source_conn.execute(f"""
        SELECT entity_key, from_block, to_block, key, value
        FROM numeric_attributes
        WHERE entity_key IN ({placeholders}) AND from_block = ?
    """, (*entity_keys, from_block)).fetchall()
    
    # Get payloads
    payloads = source_conn.execute(f"""
        SELECT entity_key, from_block, to_block, payload, content_type, string_attributes, numeric_attributes
        FROM payloads
        WHERE entity_key IN ({placeholders}) AND from_block = ?
    """, (*entity_keys, from_block)).fetchall()
    
    return {
        "entity_keys": entity_keys,
        "string_attrs": string_attrs,
        "numeric_attrs": numeric_attrs,
        "payloads": payloads,
    }


def insert_block_data(
    dest_conn: sqlite3.Connection,
    block_data: dict,
    new_block_num: int,
) -> tuple[int, int, int]:
    """Insert block data with fresh entity keys. Returns (num_entities, num_attrs, payload_kb)."""
    
    # Create mapping from old entity_key to new fresh key
    key_mapping = {}
    for old_key in block_data["entity_keys"]:
        new_key = uuid.uuid4().bytes  # Fresh 16-byte UUID
        key_mapping[old_key] = new_key
    
    cursor = dest_conn.cursor()
    num_attrs = 0
    payload_bytes = 0
    
    # Insert string attributes with fresh keys
    for row in block_data["string_attrs"]:
        old_key, _, to_block, key, value = row
        new_key = key_mapping[old_key]
        cursor.execute(
            "INSERT INTO string_attributes (entity_key, from_block, to_block, key, value) VALUES (?, ?, ?, ?, ?)",
            (new_key, new_block_num, to_block, key, value)
        )
        num_attrs += 1
    
    # Insert numeric attributes with fresh keys
    for row in block_data["numeric_attrs"]:
        old_key, _, to_block, key, value = row
        new_key = key_mapping[old_key]
        cursor.execute(
            "INSERT INTO numeric_attributes (entity_key, from_block, to_block, key, value) VALUES (?, ?, ?, ?, ?)",
            (new_key, new_block_num, to_block, key, value)
        )
        num_attrs += 1
    
    # Insert payloads with fresh keys
    for row in block_data["payloads"]:
        old_key, _, to_block, payload, content_type, str_attrs, num_attrs_json = row
        new_key = key_mapping[old_key]
        cursor.execute(
            """INSERT INTO payloads (entity_key, from_block, to_block, payload, content_type, 
               string_attributes, numeric_attributes) VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (new_key, new_block_num, to_block, payload, content_type, str_attrs, num_attrs_json)
        )
        payload_bytes += len(payload) if payload else 0
    
    return (len(block_data["entity_keys"]), num_attrs, payload_bytes // 1024)


def get_current_block_count(dest_conn: sqlite3.Connection) -> int:
    """Get the current highest block number in destination database."""
    try:
        result = dest_conn.execute("SELECT MAX(from_block) FROM payloads").fetchone()
        return result[0] if result[0] is not None else 0
    except sqlite3.OperationalError:
        return 0


def main():
    if len(sys.argv) != 5:
        print("Usage: python -m db.08_benchmark_sampled_blocks <sample_db> <base_db> <output_db> <num_blocks>")
        print()
        print("Arguments:")
        print("  sample_db  - Source database to sample blocks from (e.g., mendoza dump)")
        print('  base_db    - Starting database ("" for fresh start, or existing DB path)')
        print("  output_db  - Output database (will be created or appended to)")
        print("  num_blocks - Number of blocks to add")
        print()
        print("Examples:")
        print('  # Fresh start')
        print('  python -m db.08_benchmark_sampled_blocks ./data/mendoza.db "" ./data/sampled_100.db 100')
        print()
        print('  # Continue from existing into new file')
        print('  python -m db.08_benchmark_sampled_blocks ./data/mendoza.db ./data/sampled_100.db ./data/sampled_200.db 100')
        sys.exit(1)
    
    sample_db_path = Path(sys.argv[1])
    base_db_arg = sys.argv[2]
    output_db_path = Path(sys.argv[3])
    num_blocks = int(sys.argv[4])
    csv_path = Path(str(output_db_path) + ".csv")
    
    # Validate sample_db exists
    if not sample_db_path.exists():
        print(f"Error: Sample database not found: {sample_db_path}")
        sys.exit(1)
    
    # Handle base_db: "" means fresh start, otherwise copy if different from output
    base_db_path = None if base_db_arg == "" else Path(base_db_arg)
    
    if base_db_path is not None and not base_db_path.exists():
        print(f"Error: Base database not found: {base_db_path}")
        sys.exit(1)
    
    print("=" * 70)
    print("Sampled Block Benchmark")
    print("=" * 70)
    print(f"Sample DB:     {sample_db_path}")
    print(f"Base DB:       {base_db_path if base_db_path else '(fresh start)'}")
    print(f"Output DB:     {output_db_path}")
    print(f"CSV file:      {csv_path}")
    print(f"Blocks to add: {num_blocks}")
    print()
    
    # Connect to source for sampling
    source_conn = sqlite3.connect(str(sample_db_path))
    source_conn.row_factory = sqlite3.Row
    
    # Get valid blocks
    print("Finding valid blocks (1-500 entities)...")
    valid_blocks = get_valid_blocks(source_conn)
    print(f"Found {len(valid_blocks):,} valid blocks")
    
    if len(valid_blocks) < num_blocks:
        print(f"Warning: Only {len(valid_blocks)} valid blocks available, reducing sample size")
        num_blocks = len(valid_blocks)
    
    # Sample blocks randomly
    sampled_blocks = random.sample(valid_blocks, num_blocks)
    print(f"Sampled {num_blocks} blocks")
    print()
    
    # Setup output database
    output_db_path.parent.mkdir(parents=True, exist_ok=True)
    
    # If base_db provided and different from output, copy it first
    if base_db_path is not None and base_db_path != output_db_path:
        print(f"Copying {base_db_path} -> {output_db_path}...")
        shutil.copy2(base_db_path, output_db_path)
        # Also copy WAL/SHM files if they exist
        for suffix in ["-wal", "-shm"]:
            src = Path(str(base_db_path) + suffix)
            if src.exists():
                shutil.copy2(src, Path(str(output_db_path) + suffix))
    
    is_new_db = not output_db_path.exists()
    
    dest_conn = sqlite3.connect(str(output_db_path))
    dest_conn.execute("PRAGMA journal_mode=WAL")
    dest_conn.execute("PRAGMA synchronous=NORMAL")
    
    create_arkiv_schema(dest_conn)
    
    # Get starting block number
    start_block = get_current_block_count(dest_conn) + 1
    print(f"Starting at block {start_block}")
    
    # Setup CSV file
    write_header = not csv_path.exists() or is_new_db
    csv_file = open(csv_path, "a")
    if write_header:
        csv_file.write("block_nr,num_entities,num_attributes,payload_kb,commit_time_ms,db_size_kb\n")
    
    print()
    print("Inserting blocks...")
    print("-" * 70)
    
    total_entities = 0
    total_attrs = 0
    total_payload_kb = 0
    total_time_ms = 0
    
    for i, (source_block, entity_count) in enumerate(sampled_blocks):
        new_block_num = start_block + i
        
        # Extract block data from source
        block_data = get_block_data(source_conn, source_block)
        
        # Insert into destination and measure time
        start_time = time.perf_counter()
        num_entities, num_attrs, payload_kb = insert_block_data(dest_conn, block_data, new_block_num)
        dest_conn.commit()
        end_time = time.perf_counter()
        
        commit_time_ms = int((end_time - start_time) * 1000)
        db_size_kb = output_db_path.stat().st_size // 1024
        
        # Log to CSV (all values as integers)
        csv_file.write(f"{new_block_num},{num_entities},{num_attrs},{payload_kb},{commit_time_ms},{db_size_kb}\n")
        csv_file.flush()
        
        total_entities += num_entities
        total_attrs += num_attrs
        total_payload_kb += payload_kb
        total_time_ms += commit_time_ms
        
        # Progress
        if (i + 1) % 100 == 0 or i == num_blocks - 1:
            print(f"  Block {new_block_num}: {i+1}/{num_blocks} done, DB size: {db_size_kb/1024:.1f} MB")
    
    csv_file.close()
    dest_conn.close()
    source_conn.close()
    
    # Final stats
    final_db_size_kb = output_db_path.stat().st_size // 1024
    
    print()
    print("=" * 70)
    print("RESULTS")
    print("=" * 70)
    print(f"Blocks inserted:    {num_blocks:,}")
    print(f"Total entities:     {total_entities:,}")
    print(f"Total attributes:   {total_attrs:,}")
    print(f"Total payload:      {total_payload_kb/1024:.1f} MB")
    print(f"Total time:         {total_time_ms/1000:.1f} s")
    print(f"Final DB size:      {final_db_size_kb/1024:.1f} MB")
    print()
    print(f"Avg entities/block: {total_entities/num_blocks:.1f}")
    print(f"Avg attrs/block:    {total_attrs/num_blocks:.1f}")
    print(f"Avg time/block:     {total_time_ms/num_blocks:.1f} ms")
    print(f"Entities/second:    {total_entities/(total_time_ms/1000):.0f}")
    print("=" * 70)
    print()
    print(f"CSV file: {csv_path}")


if __name__ == "__main__":
    main()
