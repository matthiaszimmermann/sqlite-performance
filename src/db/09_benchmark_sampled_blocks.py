#!/usr/bin/env python3
"""Benchmark using sampled blocks from real arkiv data (logs string/numeric attribute counts separately).

Derived from 08_benchmark_sampled_blocks.py.
...
Usage:
    uv run python -m db.09_benchmark_sampled_blocks <sample_db> <base_db> <output_db> <num_blocks>
...
def insert_block_data(
    ...
    """Insert block data with fresh entity keys. Returns (num_entities, num_attrs, payload_kb, num_str_attrs, num_num_attrs)."""
    key_mapping = {}
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python -m db.09_benchmark_sampled_blocks <sample_db> <base_db> <output_db> <num_blocks>")
        print()
        print("Arguments:")
        ...
        print("Examples:")
        print('  # Fresh start')
        print('  python -m db.09_benchmark_sampled_blocks ./data/mendoza.db "" ./data/sampled_100.db 100')
        print()
        print('  # Continue from existing into new file')
        print('  python -m db.09_benchmark_sampled_blocks ./data/mendoza.db ./data/sampled_100.db ./data/sampled_200.db 100')
        sys.exit(1)
    ...
    if base_db_path is not None and not base_db_path.exists():
        ...
        sys.exit(1)
    print("=" * 70)
    print("Sampled Block Benchmark (string/numeric attr counts)")
    print("=" * 70)
